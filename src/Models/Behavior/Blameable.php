...
namespace Parrot\Models\Behavior;

use Phalcon\Mvc\Model;
use Phalcon\Mvc\ModelInterface;
use Phalcon\Mvc\Model\Behavior;
use Phalcon\Mvc\Model\BehaviorInterface;
use Parrot\Models\Audits;
use Parrot\Models\AuditDetails;
use Parrot\Models\AuditDetailInterface;
use Parrot\Models\AuditInterface;
...
class Blameable extends Behavior implements BehaviorInterface
{
     ...
     * @var string
     */
    protected $auditClass = Audits::class;

    /**
     * @var string
     */
    protected $auditDetailClass = AuditDetails::class;
    ...
    public function __construct($options = null)
    {
        ...
        if (isset($options['auditClass'])) {
            if (!in_array(AuditInterface::class, class_implements($options['auditClass']))) {
                throw new Exception(
                    "Your class must implement Phalcon\\Mvc\\Model\\AuditInterface"
                );
            }
        ...
        if (isset($options['auditDetailClass'])) {
            if (!in_array(AuditDetailInterface::class, class_implements($options['auditDetailClass']))) {
                throw new Exception(
                    "Your class must implement Phalcon\\Mvc\\Model\\AuditDetailInterface"
                );
            }
    ...
    public function auditAfterUpdate(ModelInterface $model)
    {
        ...
        if (false == $result) {
            $message = null;
            foreach ($audit->getMessages() as $m) {
                $message  = $m;
                echo $message;
            }
        }
